      ...
      <div className="digest-shell">
        ...
        <div className="digest-sections">
          {sections.map((section) => (
            <DashboardSection key={section.id} title={section.title} items={section.items} />
          ))}
        </div>
      </div>
    </main>
...
export function buildSections(feed: FeedResponse): DashboardSectionData[] {
  return [
    { id: 'now', title: 'Now', items: sortSectionFeedItems(feed.now).map(toSectionItem) },
    { id: 'today', title: 'Today', items: sortSectionFeedItems(feed.today).map(toSectionItem) },
    { id: 'later', title: 'Later', items: sortSectionFeedItems(feed.worth_knowing).map(toSectionItem) },
  ];
}
