...
WORKSPACE="${ROOT_DIR}/ios/DecisionPipeline/DecisionPipeline.xcworkspace"
PROJECT_FILE="${ROOT_DIR}/ios/DecisionPipeline/Project.swift"
SCHEME="${IOS_SCHEME:-DecisionPipeline}"
BUNDLE_ID="${IOS_BUNDLE_ID:-com.rameshpandey.DecisionPipeline}"
...
if [[ ! -d "${WORKSPACE}" ]]; then
  ...
  (cd "${ROOT_DIR}/ios/DecisionPipeline" && tuist generate)
