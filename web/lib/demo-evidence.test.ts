...
test('demo evidence returns item-scoped raw Gmail records', () => {
  const records = getDemoSourceRecords('hdfc-card-bill');

  assert.equal(records.length, 2);
  assert.equal(records[0].thread_id, 'hdfc-card-statement-feb');
  assert.equal(records[0].raw_payload.detected_item_id, 'hdfc-card-bill');
  assert.match(String(records[0].raw_payload.subject), /HDFC credit card/);
});

test('demo trace replays the frontend evidence pipeline for one entity', () => {
  const trace = getDemoTrace('entity-hdfc-card-bill');

  assert.equal(trace?.entity_id, 'entity-hdfc-card-bill');
  assert.deepEqual(
...
test('demo thread honors the same pagination contract as the backend', () => {
  const firstPage = getDemoThread('entity-hdfc-card-bill', { limit: 1, offset: 0 });
  const secondPage = getDemoThread('entity-hdfc-card-bill', { limit: 1, offset: 1 });
