...
test('command palette cache version changes for smart inbox command source', () => {
  assert.match(commandPaletteSource, /decision-pipeline-command-index:v5/);
  assert.doesNotMatch(commandPaletteSource, /decision-pipeline-command-index:v4/);
});
