...
CANONICAL_IDENTITY_RULES = (
    CanonicalIdentityRule("Porkbun", ("porkbun.com",), ("porkbun",)),
    ...
    CanonicalIdentityRule(
        "HDFC Bank",
        ("hdfc.bank.in", "hdfcbank.bank.in", "hdfcbank.com"),
        ("hdfc", "hdfc bank"),
    ),
    CanonicalIdentityRule(
        "HSBC",
        ("hsbc.co.in", "hsbc.bank.in", "hsbc.com"),
        ("hsbc",),
    ),
    CanonicalIdentityRule(
        "SBI",
    ...
    CanonicalIdentityRule(
        ...
        ("incometax.gov.in",),
        ("income tax department", "your itr intimation", "income tax"),
    ),
    CanonicalIdentityRule(
        "AI Grants India",
        ("aigrants.in",),
...
def counterpart_aware_headline(title: str, counterpart_entities: Sequence[str]) -> str:
    ...
    primary = entities[0]
    if primary == "HDFC Bank":
        primary_pattern = r"HDFC(?:\s+Bank)?"
    else:
        suffix = r"(?:\s+Bank)?" if primary in {"HDFC", "HSBC", "SBI"} else ""
    ...
    # verbs add no meaning: `HDFC · Confirms X` is clearer as `HDFC · X`.
...
def _domain_backed_presentation(
    ...
    *,
    ...
    A bare custom-domain sender such as ``support@rbiretaildirect.org.in``
    deterministically identifies ``rbiretaildirect`` but does not reveal its
