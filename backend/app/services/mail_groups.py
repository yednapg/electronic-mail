...
def _smart_workflow_platform(tokens: set[str], *, category: str) -> str | None:
    ...
    if category == "wire-remittance" and "northstar" in tokens:
        return "northstar-bank"
...
def _mailbox_support_reference_outcome_title(title: str, display_value: str) -> str | None:
    ...
    match = re.match(
        r"(?i)^(?P<provider>Northstar(?:\s+Bank)?)\s+(?:confirms\s+)?(?:outward\s+)?remittance\s+processed\b",
        cleaned,
    )
    if match:
        provider = "Northstar"
    ...
    try:
        ...
        prompt = {
            ...
            "rules": [
                ...
                "Use concrete title patterns like 'Apple order out for delivery', 'Northstar wire transfer processed', or 'Bank case 106400420 acknowledged'.",
