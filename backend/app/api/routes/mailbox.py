...
def mailbox(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="An error occurred while processing the request.") from exc
...
def mailbox_attachment(request: Request, message_id: str, attachment_id: str) -> Response:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail="An error occurred while processing the request.") from exc
    except Exception as exc:
...
def mailbox_thread_action(request: Request, payload: QueuedThreadActionRequest) -> QueuedThreadActionResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="An error occurred while processing the request.") from exc
