...
def patch_ai_organization_profile(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="An error occurred while processing the request.") from exc
    if generation_id:
        enqueue_generation_bootstrap(
            settings,
            user_id=user.id,
...
def evaluate_shadow_generation(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="An error occurred while processing the request.") from exc
