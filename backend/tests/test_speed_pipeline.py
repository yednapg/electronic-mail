...
def sample_visible_group() -> VisibleMailGroupRecord:
    return VisibleMailGroupRecord(
        ...
        contact_channel="HDFC Bank Care",
        title="HDFC remittance support case",
        summary="HDFC acknowledged and updated the wire transfer inquiry.",
...
class SpeedPipelineTests(unittest.TestCase):
    ...
    def test_projection_refresh_persists_ai_lifecycle_proposals(
        ...
        inbound = replace(
            ...
            gmail_thread_id="thread-1",
            sender="HDFC Bank <support@hdfcbank.com>",
            subject="Request for Status of International Wire Sent",
            snippet="HDFC is checking the international wire status.",
            extracted_signals={"sender_domain": "hdfcbank.com"},
        )
        processed = replace(
            sample_message("msg-2"),
            gmail_thread_id="thread-2",
            sender="Tradequalityunit <tradequalityunit@hdfcbank.bank.in>",
            subject="Outward remittance processed",
            snippet="The outward remittance was processed.",
            extracted_signals={"sender_domain": "hdfcbank.bank.in"},
        )
        sent = replace(
            sample_message("msg-3"),
            gmail_thread_id="thread-3",
            label_ids=["SENT"],
            sender="Gaurav Pandey <me@example.com>",
        ...
        mock_proposals.return_value = [
            {
                ...
                "per_message_evidence": {
                    "msg-1": "HDFC support is responding on the wire status request.",
                    "msg-2": "HDFC confirms the remittance was processed.",
                },
                ...
                "terminal_state": True,
                "urgency": "low",
                "ai_title": "HDFC international wire remittance status",
                "ai_summary": "HDFC support and remittance mail describe the same international wire status.",
            }
        ]
    ...
    def test_mailbox_uses_thread_ai_group_for_plain_gmail_thread_rows(self) -> None:
        message = replace(
            ...
            internal_date="2026-05-15T13:00:00+00:00",
            subject="HDFC support case 106400420 acknowledged",
            sender="HDFC Bank <support@hdfcbank.com>",
            snippet="HDFC confirmed support case 106400420 and next steps.",
            extracted_signals={
                "sender_domain": "hdfcbank.com",
                "ticket_id": "106400420",
                "normalized_subject": "hdfc support case 106400420 acknowledged",
            },
        )
        ai_group = replace(
            sample_group(),
            ...
            ai_title="HDFC support case 106400420 acknowledged",
        ...
        self.assertEqual(rows[0].title, "HDFC support case 106400420 acknowledged")
    ...
    def test_ai_group_title_is_ready_without_persisted_summary(self) -> None:
        message = replace(
            ...
            subject="HDFC wire remittance processed",
            sender="HDFC Bank <support@hdfcbank.com>",
        ...
        ai_group = replace(
            ...
            ai_title="HDFC wire remittance processed",
            ai_summary="",
        )

        row = _gmail_row_from_canonical_thread("thread-title-only", [message], "inbox", ai_group)

        self.assertEqual(row.title, "HDFC wire remittance processed")
        self.assertEqual(row.ai_title, "HDFC wire remittance processed")
    ...
    def test_ai_batch_rejects_cross_thread_collection_without_consuming_messages(self) -> None:
        ...
        penn = replace(
            ...
            sender="Penn State <admissions@psu.edu>",
            subject="Your Penn State application was updated",
            snippet="Your application and cost estimate were updated.",
            extracted_signals={"sender_domain": "psu.edu", "normalized_subject": "application update"},
        )
        emergent = replace(
    ...
    def test_ai_batch_rejects_cross_thread_lifecycle_without_complete_evidence(self) -> None:
        ...
        opened = replace(
            ...
            extracted_signals={"sender_domain": "hdfcbank.com", "ticket_id": "106400420"},
        )
        updated = replace(
        ...
        output = {
            ...
            "ai_title": "HDFC service request 106400420 under review",
    ...
    def test_single_message_ai_group_prefers_message_title_when_group_title_has_bare_acronym_sender(self) -> None:
        ...
        rows = [row for section in mailbox.sections for row in section.rows]
        self.assertEqual(rows[0].sender, "Penn State")
        self.assertEqual(rows[0].title, "Penn State limited services PIN")
        self.assertEqual(rows[0].ai_title, "Penn State limited services PIN")
        self.assertEqual(rows[0].children[0].ai_title, "Penn State limited services PIN")
    ...
    def test_mailbox_display_keeps_safe_provider_stream_with_sender_variants_separate_in_primary_inbox(self) -> None:
        weekly = replace(
            ...
            sender='"Harsha @ Slashy" <founders@mail.slashy.com>',
            subject="Your meeting notes and your CRM now live in your inbox (Slashy Weekly #4)",
            snippet="Slashy Weekly product update.",
            extracted_signals={"sender_domain": "mail.slashy.com", "domains": ["slashy.com", "www.slashy.com"]},
        )
        welcome = replace(
            ...
            internal_date="2026-05-24T12:00:00+00:00",
            sender="Slashy Team <founders@slashy.com>",
            subject="Welcome to Slashy!",
            snippet="We're very excited to have you on Slashy.",
            extracted_signals={"sender_domain": "slashy.com", "normalized_subject": "welcome to slashy"},
        )
    ...
    def test_mailbox_renders_hdfc_workflow_ai_group_across_gmail_threads(self) -> None:
        first = replace(
            sample_message("hdfc-ack"),
            gmail_thread_id="thread-hdfc-ack",
            internal_date="2026-05-15T12:00:00+00:00",
            subject="HDFC Bank acknowledged your service request",
            sender="HDFC Bank <support@hdfcbank.com>",
            ai_title="Bank acknowledged service request",
        )
        second = replace(
            sample_message("hdfc-update"),
            gmail_thread_id="thread-hdfc-update",
            internal_date="2026-05-15T13:00:00+00:00",
            subject="Update on your HDFC wire transfer inquiry",
            sender="HDFC Bank <support@hdfcbank.com>",
        ...
        with patch("app.services.mail_groups.ensure_background_import_work"), patch(
            ...
            "app.services.mail_groups.list_mailbox_thread_page",
            return_value=MailboxThreadPage(threads=[("thread-hdfc-update", [second]), ("thread-hdfc-ack", [first])], next_cursor=None, loaded_threads=2),
        ), patch(
            "app.services.mail_groups.list_visible_groups_for_gmail_threads",
            return_value={"thread-hdfc-update": visible_group, "thread-hdfc-ack": visible_group},
        ), patch(
            "app.services.mail_groups.list_mail_groups_for_gmail_threads",
            return_value={},
        ), patch(
            "app.services.mail_groups.list_messages_for_visible_groups",
            return_value={"visible-hdfc": [first, second]},
        ), patch(
            "app.services.mail_groups.count_mailbox_threads",
            return_value=2,
        ), patch("app.services.mail_groups.oldest_imported_message_at", return_value=None), patch(
        ...
        rows = [row for section in mailbox.sections for row in section.rows]
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].thread_id, "group-hdfc")
        self.assertEqual(rows[0].entity_id, "visible-hdfc")
        self.assertEqual(rows[0].ai_group_id, "group-hdfc")
        self.assertEqual(rows[0].sender, "HDFC Bank")
        self.assertEqual(rows[0].title, "HDFC remittance support case")
        self.assertEqual(rows[0].summary, "HDFC acknowledged and updated the wire transfer inquiry.")
        self.assertEqual(rows[0].latest_subject, "Update on your HDFC wire transfer inquiry")
        self.assertEqual(rows[0].message_count, 2)
        self.assertEqual([child.message_id for child in rows[0].children], ["hdfc-ack", "hdfc-update"])
        self.assertEqual([child.gmail_thread_id for child in rows[0].children], ["thread-hdfc-ack", "thread-hdfc-update"])

    def test_visible_group_service_channel_canonical_uses_provider_sender(self) -> None:
        first = replace(
            sample_message("hdfc-grievance-ack"),
            gmail_thread_id="thread-hdfc-grievance-ack",
            label_ids=["INBOX"],
            internal_date="2026-05-30T12:00:00+00:00",
            subject="HDFC Bank acknowledged your grievance",
            sender="Grievance-Redressalcc <grievance.redressalcc@hdfc.bank.in>",
            ai_title="HDFC grievance acknowledged",
        )
        second = replace(
            sample_message("hdfc-grievance-update"),
            gmail_thread_id="thread-hdfc-grievance-update",
            label_ids=["INBOX"],
            internal_date="2026-06-05T13:00:00+00:00",
            subject="Update on your HDFC grievance",
            sender="Grievance-Redressalcc <grievance.redressalcc@hdfc.bank.in>",
            ai_title="HDFC grievance updated",
        )
        visible_group = replace(
            sample_visible_group(),
            id="visible-hdfc-grievance",
            source_group_id="group-hdfc-grievance",
        ...
        with patch("app.services.mail_groups.ensure_background_import_work"), patch(
            ...
            "app.services.mail_groups.list_mailbox_thread_page",
            return_value=MailboxThreadPage(threads=[("thread-hdfc-grievance-update", [second]), ("thread-hdfc-grievance-ack", [first])], next_cursor=None, loaded_threads=2),
        ), patch(
            "app.services.mail_groups.list_visible_groups_for_gmail_threads",
            return_value={"thread-hdfc-grievance-update": visible_group, "thread-hdfc-grievance-ack": visible_group},
        ), patch(
            "app.services.mail_groups.list_mail_groups_for_gmail_threads",
            return_value={},
        ), patch(
            "app.services.mail_groups.list_messages_for_visible_groups",
            return_value={"visible-hdfc-grievance": [first, second]},
        ), patch(
            "app.services.mail_groups.count_mailbox_threads",
            return_value=2,
        ), patch("app.services.mail_groups.oldest_imported_message_at", return_value=None), patch(
        ...
        self.assertEqual([child.sender for child in rows[0].children], ["HDFC Bank", "HDFC Bank"])
    ...
    def test_visible_group_acronym_canonical_uses_repeated_ai_organization_title(self) -> None:
        first = replace(
            ...
            sender="admissions@psu.edu",
            ai_title="Penn State application update",
            extracted_signals={"sender_domain": "psu.edu", "normalized_subject": "application update"},
        )
        second = replace(
        ...
        visible_group = replace(
            ...
            summary="Penn State sent multiple application update emails.",
        ...
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].sender, "Penn State")
        self.assertEqual(rows[0].title, "Penn State application update")
        self.assertEqual([child.sender for child in rows[0].children], ["Penn State", "Penn State"])
    ...
    def test_visible_group_sender_falls_back_to_inbound_sender_when_projection_entity_is_user(self) -> None:
        sent = replace(
            ...
            sender="Gaurav Pandey <hi@example.com>",
            recipients={"to": "Penn State <admissions@psu.edu>"},
        )
        reply = replace(
        ...
        visible_group = replace(
            ...
            title="Penn State reconsideration request",
            summary="Penn State replied to the reconsideration request.",
            canonical_entity="Gaurav Pandey",
        )
        ...
        rows = [row for section in mailbox.sections for row in section.rows]
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].latest_sender, "Penn State <admissions@psu.edu>")
        self.assertEqual(rows[0].sender, "Penn State")
        self.assertEqual(rows[0].title, "Penn State reconsideration request")
    ...
    def test_attention_title_does_not_turn_application_arrival_into_delivery_status(self) -> None:
        message = replace(
            ...
            extracted_signals={"sender_domain": "psu.edu", "normalized_subject": "i-20 request needed"},
        )
        ...
        enrichment = attention_enrichment_payload(
            ...
            ai_output={
                ...
                "ai_title": "Penn State I-20 request needed arriving",
            },
        )

        self.assertEqual(enrichment["ai_title"], "Penn State I-20 request needed")

    def test_attention_title_replaces_pending_response_with_resolved(self) -> None:
        message = replace(
            sample_message("hdfc-grievance"),
            subject="HDFC grievance 105715521 resolved",
            sender="HDFC Bank <support@hdfcbank.com>",
            ...
            extracted_signals={"sender_domain": "hdfcbank.com", "ticket_id": "105715521"},
        )

        enrichment = attention_enrichment_payload(
            messages=[message],
            group_key="gmail-thread:hdfc-grievance",
            ai_output={
                "workflow_family": "support_case",
                "workflow_state": "resolved",
                "requires_user_action": False,
                ...
                "urgency": "low",
                "confidence": 0.96,
                "reason": "HDFC marked the grievance resolved.",
                "ai_title": "HDFC grievance 105715521 pending response",
            },
        )

        self.assertEqual(enrichment["ai_title"], "HDFC grievance 105715521 resolved")
    ...
    def test_attention_title_does_not_stack_confirmed_onto_ready_or_successful_titles(self) -> None:
        statement = replace(
            ...
            sender="Interactive Brokers <statements@interactivebrokers.com>",
            ...
            extracted_signals={"sender_domain": "interactivebrokers.com", "normalized_subject": "daily activity statement ready"},
        )
        order = replace(
    ...
    def test_attention_title_does_not_append_reply_to_answered_reconsideration(self) -> None:
        message = replace(
            ...
            sender="Penn State Behrend <admissions@behrend.psu.edu>",
            ...
            extracted_signals={"sender_domain": "psu.edu", "normalized_subject": "reconsideration request"},
        )
        ...
        enrichment = attention_enrichment_payload(
            ...
            ai_output={
                ...
                "urgency": "low",
                "confidence": 0.93,
                "reason": "Penn State replied to the reconsideration request.",
                "ai_title": "Penn State Behrend reconsideration request answered",
            },
        )

        self.assertEqual(enrichment["ai_title"], "Penn State Behrend reconsideration request answered")
    ...
    def test_ai_group_title_replaces_status_with_resolved_outcome(self) -> None:
        opened = replace(
            sample_message("hdfc-opened"),
            gmail_thread_id="thread-hdfc-opened",
            sender="HDFC Bank <support@hdfcbank.com>",
            subject="Request for Status of International Wire Sent",
            snippet="HDFC acknowledged the international wire status request.",
            text_body="HDFC acknowledged the international wire status request.",
            extracted_signals={"sender_domain": "hdfcbank.com"},
        )
        processed = replace(
            sample_message("hdfc-processed"),
            gmail_thread_id="thread-hdfc-processed",
            sender="HDFC Bank <tradequalityunit@hdfcbank.bank.in>",
            subject="Outward remittance processed",
            snippet="The outward remittance was processed.",
        ...
        enrichment = attention_enrichment_payload(
            ...
            ai_output={
                ...
                "urgency": "low",
                "confidence": 0.97,
                "reason": "HDFC confirmed the remittance was processed.",
                "ai_title": "HDFC international wire remittance status",
                "ai_summary": "HDFC acknowledged the wire request and later confirmed the outward remittance was processed.",
            },
        )

        self.assertEqual(enrichment["ai_title"], "HDFC international wire remittance processed")
    ...
    def test_mailbox_display_keeps_safe_job_alert_streams_separate_in_primary_inbox(self) -> None:
        ...
        second = replace(
            ...
            sender="Upwork Notification <donotreply@upwork.com>",
            snippet="This job matches your alert settings. Payment verified client.",
            extracted_signals={"sender_domain": "upwork.com", "normalized_subject": "new job alert ai consultant"},
        )
    ...
    def test_mailbox_keeps_psu_application_reconsideration_and_costs_as_separate_threads(self) -> None:
        ...
        reply = replace(
            ...
            subject="RE: Question About Reconsideration Request",
            sender="Penn State Behrend <behrend.admissions@psu.edu>",
        )
        costs = replace(
        ...
        with ExitStack() as stack:
            stack.enter_context(
                patch(
                    ...
                    return_value=SimpleNamespace(id="user-1", email="me@example.com", display_name="Gaurav"),
                )
            )
            stack.enter_context(
                patch(
