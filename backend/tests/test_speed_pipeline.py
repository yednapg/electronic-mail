...
class SpeedPipelineTests(unittest.TestCase):
    ...
    def test_candidate_groups_normalize_exact_reference_sender_variants(self) -> None:
        ...
        self.assertEqual(list(candidates), ["ticket_id:provider:900000003"])
        self.assertEqual([message.message_id for message in candidates["ticket_id:provider:900000003"]], ["provider-grievance", "provider-service"])
    ...
    def test_mailbox_uses_compatible_ai_group_for_title_and_summary_without_replacing_thread_id(self) -> None:
        ...
        self.assertEqual(rows[0].grouping_metadata["evidence"]["strong_evidence"], ["ticket_id:900000002"])
    ...
    def test_mailbox_uses_thread_ai_group_for_plain_gmail_thread_rows(self) -> None:
        message = replace(
            ...
            internal_date="2026-05-15T13:00:00+00:00",
            subject="Northstar support case 900000002 acknowledged",
            sender="Northstar Bank <support@northstarbank.example>",
            snippet="Northstar confirmed support case 900000002 and next steps.",
            extracted_signals={
                "sender_domain": "northstarbank.example",
                "ticket_id": "900000002",
                "normalized_subject": "northstar support case 900000002 acknowledged",
            },
        )
        ai_group = replace(
            sample_group(),
            ...
            ai_title="Northstar support case 900000002 acknowledged",
        ...
        self.assertEqual(rows[0].title, "Northstar support case 900000002 acknowledged")
    ...
    def test_ai_batch_rejects_cross_thread_lifecycle_without_complete_evidence(self) -> None:
        ...
        opened = replace(
            ...
            sender="Northstar Bank <support@northstarbank.example>",
            subject="Service request 900000002 registered",
            snippet="Service request number 900000002 has been registered.",
            extracted_signals={"sender_domain": "northstarbank.example", "ticket_id": "900000002"},
        )
        updated = replace(
            sample_message("case-updated"),
            gmail_thread_id="thread-case-updated",
            sender="Northstar Bank <support@northstarbank.example>",
            subject="Service request 900000002 update",
            snippet="Service request number 900000002 is being reviewed.",
            extracted_signals={"sender_domain": "northstarbank.example", "ticket_id": "900000002"},
        )
        output = {
            ...
            "group_kind": "lifecycle",
            "canonical_entity": "Northstar Bank",
            "shared_object": "service request 900000002",
            "workflow_family": "support_case",
            "workflow_state": "waiting",
            "confidence": 0.97,
            "risk_level": "low",
            "should_show_in_inbox": True,
            "strong_evidence": ["service request 900000002"],
            "per_message_evidence": {"case-opened": "Registered the service request."},
            "ai_title": "Northstar service request 900000002 under review",
    ...
    def test_ai_batch_stores_high_confidence_cross_thread_lifecycle_as_real_workflow(self) -> None:
        ...
        output = {
            ...
            "strong_evidence": ["service request 900000002"],
            "per_message_evidence": {
                "case-opened": "Registered service request 900000002.",
                "case-updated": "Updated the same service request 900000002.",
            },
            "ai_title": "Northstar service request 900000002 under review",
            "member_message_ids": ["case-opened", "case-updated"],
        }
    ...
    def test_mailbox_display_does_not_provider_stream_cluster_support_auto_acks(self) -> None:
        ...
        second = replace(
            ...
            sender="Support Department <grievance.redressalcc@northstar.example>",
            subject="<Auto> '1000000002' Unauthorized Credit Card Consent Request and Data Privacy Concern",
    ...
    def test_mailbox_display_clusters_exact_reference_without_topic_words(self) -> None:
        opened = replace(
            ...
            ai_title="Bank registered service request 900000002",
            snippet="The request is registered and our team is working on it.",
            extracted_signals={"sender_domain": "bank.example.com", "ticket_id": "900000002"},
        )
        updated = replace(
            sample_message("case-updated"),
            gmail_thread_id="thread-case-updated",
            ...
            ai_title="Bank updated service request 900000002",
        ...
        self.assertEqual(rows[0].title, "Bank Support service request 900000002 updated")
        self.assertEqual(
            rows[0].summary,
            "Service request 900000002: 2 Bank Support updates; latest: Bank updated service request 900000002.",
        )
        self.assertEqual(rows[0].grouping_metadata["reference"]["value"], "900000002")
    ...
    def test_mailbox_display_bridges_no_ticket_auto_ack_into_exact_reference_cluster(self) -> None:
        ...
        registered = replace(
            ...
            snippet="The unique reference number is 900000003.",
            extracted_signals={
                "sender_domain": "bank.example.com",
        ...
        updated = replace(
            ...
            snippet="We have updated service request 900000003.",
            extracted_signals={
                "sender_domain": "bank.example.com",
        ...
        self.assertEqual(len(rows), 1)
        self.assertTrue(rows[0].thread_id.startswith("mailbox-cluster:"))
        self.assertEqual(rows[0].title, "Bank Support service request 900000003 updated")
    ...
    def test_mailbox_display_does_not_bridge_conflicting_visible_reference_into_exact_cluster(self) -> None:
        conflicting_auto_ack = replace(
            ...
            internal_date="2026-06-01T12:00:00+00:00",
            sender="Bank Support <support@bank.example.com>",
            subject="<Auto> '1000000003' Privacy complaint received",
            snippet="Your complaint reference 1000000003 has been received and is under review.",
            extracted_signals={
                "sender_domain": "bank.example.com",
                "normalized_subject": "auto 1000000003 privacy complaint received",
                "references": "<root@example.com> <bridge@example.com>",
            },
            headers={"references": "<root@example.com> <bridge@example.com>"},
        )
        ...
        self.assertEqual(clustered_rows[0].title, "Bank Support service request 900000003 updated")
    ...
    def test_visible_group_service_channel_canonical_uses_provider_sender(self) -> None:
        ...
        visible_group = replace(
            ...
            title="Northstar Bank grievance case 900000001",
        ...
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].sender, "Northstar Bank")
        self.assertEqual(rows[0].title, "Northstar Bank grievance case 900000001")
    ...
    def test_attention_title_replaces_pending_response_with_resolved(self) -> None:
        message = replace(
            ...
            subject="Northstar grievance 900000001 resolved",
            sender="Northstar Bank <support@northstarbank.example>",
            snippet="Your grievance 900000001 has been resolved and closed.",
            text_body="Your grievance 900000001 has been resolved and closed.",
            extracted_signals={"sender_domain": "northstarbank.example", "ticket_id": "900000001"},
        )
        ...
        enrichment = attention_enrichment_payload(
            ...
            ai_output={
                ...
                "ai_title": "Northstar grievance 900000001 pending response",
            },
        )

        self.assertEqual(enrichment["ai_title"], "Northstar grievance 900000001 resolved")
