...
class MailGroupDetailBodyTests(unittest.TestCase):
    def test_synthetic_group_detail_expands_member_gmail_threads(self) -> None:
        ...
        group = replace(sample_group(), id="group-ai", group_key="ai:psu", membership_source="ai_batch", ai_title="Penn State application updates")
        sent = replace(
            sample_message(),
            message_id="sent-1",
    ...
    def test_smart_row_detail_returns_messages_from_group_reader_target(self) -> None:
        ...
        wire = replace(
            ...
            subject="HDFC international wire registered",
        )
        reply = replace(
            sample_message(),
            ...
            subject="HDFC response on wire request",
        )
        ...
        with patch("app.services.mail_groups.get_smart_inbox_row", return_value=smart_row) as get_row, patch(
            ...
            response = build_group_detail_response(
                settings,
                user_id="user-1",
                group_id="smart-row:smart-row-hdfc-wire",
        ...
        self.assertEqual(response.gmail_thread_id, "smart-row:smart-row-hdfc-wire")
        self.assertEqual(response.title, "Wire transfer status with HDFC Bank")
    ...
    def test_smart_row_detail_generates_summary_on_request_instead_of_using_row_summary(self) -> None:
        ...
        with patch("app.services.mail_groups.get_smart_inbox_row", return_value=smart_row), patch(
            ...
            return_value="HDFC registered the wire transfer and the bank response is pending.",
        ) as mock_generate:
            response = build_group_detail_response(
                settings,
                user_id="user-1",
        ...
        self.assertIsNotNone(response)
        self.assertEqual(response.summary, "HDFC registered the wire transfer and the bank response is pending.")
