...
class SmartInboxQualityTests(unittest.TestCase):
        ...
        report = audit_smart_inbox_quality(
            _mailbox(
                _row(
                    ...
                    title="HDFC Bank service request 106400420 updated",
                    sender="HDFC Bank",
                    message_count=2,
                    grouping_metadata={
                        "source": "mailbox_display_cluster",
                        "reference": {"signal_name": "ticket_id", "value": "106400420"},
                    },
                    children=[
                        _child("registered service request 106400420"),
                        _child("updated service request 106400420"),
                    ],
                )
            )
        )
    ...
    def test_smart_inbox_response_requires_all_visible_rows_ready(self) -> None:
        smart = _smart_inbox(
            ...
            _smart_row("smart-row:partial", "HDFC transfer pending review", readiness="partial"),
            ready_count=1,
            partial_count=1,
        )
    ...
    def test_smart_cross_thread_mail_object_evidence_passes(self) -> None:
        smart = _smart_inbox(
            _smart_row(
                ...
                "HDFC Bank service request 106756996 resolved",
                source_thread_ids=["thread-1", "thread-2"],
                source_message_ids=["msg-1", "msg-2"],
                grouping_reason={
                    "source": "mail_object",
                    "canonical_key": "ticket_id:hdfc:106756996",
                    "evidence": {"signal_name": "ticket_id", "reference_value": "106756996"},
                },
            )
        )
    ...
    def test_smart_inbox_blocks_stacked_state_titles(self) -> None:
        smart = _smart_inbox(
            _smart_row("smart-row:pending-resolved", "HDFC grievance 105715521 pending response resolved"),
            _smart_row("smart-row:application-arriving", "Penn State I-20 request needed arriving"),
