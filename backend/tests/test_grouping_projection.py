...
def message(
    ...
    *,
    thread_id: str,
    sender: str = "Northstar Bank <support@northstarbank.example>",
    subject: str = "Northstar Bank service request update",
    snippet: str = "Service request 900000002 was updated.",
    ...
    return GmailMessageRecord(
        ...
        text_body=None,
        extracted_signals=signals or {"sender_domain": "northstarbank.example", "ticket_id": "900000002"},
...
def group(
    ...
    *,
    ...
    return MailGroupRecord(
        ...
        ai_title="Northstar remittance support case",
        ai_summary="Northstar updated service request 900000002.",
        labels=["support_case"],
        action_needed=False,
        action_type="open",
        priority=20,
        ...
        or {
            ...
            "facts": {"provider": "northstar", "reference_id": "ticket_id:900000002"},
        },
        classification_confidence=0.92,
    )


def ai_contract(*message_ids: str, shared_object: str = "Northstar wire transfer status case") -> dict:
    return {
...
class GroupingProjectionTests(unittest.TestCase):
    def test_exact_reference_cross_thread_group_passes(self) -> None:
        ...
        decision = evaluate_inbox_candidate(group("group-1", group_key="ticket_id:northstarbank.example:900000002"), messages)

        self.assertTrue(decision.accepted)
        self.assertEqual(decision.group_kind, "lifecycle")
        self.assertEqual(decision.canonical_entity, "Northstar Bank")
        self.assertIn("ticket_id:northstarbank.example:900000002", decision.evidence["strong_evidence"])

    def test_provider_topic_only_cross_thread_group_is_rejected(self) -> None:
        messages = [
            message("msg-1", thread_id="thread-1", signals={"sender_domain": "northstarbank.example"}),
            message("msg-2", thread_id="thread-2", signals={"sender_domain": "northstarbank.example"}),
        ]
        candidate = group(
            "group-1",
            group_key="entity-lifecycle:northstarbank.example:northstar-bank:transfer",
            classification={"workflow_family": "support_case", "facts": {"provider": "northstar"}},
        )
    ...
    def test_high_confidence_ai_lifecycle_group_passes_without_extracted_ids(self) -> None:
        messages = [
            message(
                ...
                signals={"sender_domain": "northstarbank.example"},
            ),
            message(
                "msg-2",
                thread_id="thread-2",
    ...
    def test_high_confidence_ai_lifecycle_rejects_conflicting_references(self) -> None:
        messages = [
            message("msg-1", thread_id="thread-1", signals={"sender_domain": "northstarbank.example", "ticket_id": "900000002"}),
            message("msg-2", thread_id="thread-2", signals={"sender_domain": "northstarbank.example", "ticket_id": "900000003"}),
        ]
        candidate = group(
            "group-ai",
            group_key="ai-lifecycle:northstar-conflict",
            membership_source="ai_lifecycle",
            classification=ai_contract("msg-1", "msg-2"),
        )
    ...
    def test_ai_lifecycle_sent_context_does_not_create_entity_conflict(self) -> None:
        inbound = message("msg-1", thread_id="thread-1", signals={"sender_domain": "northstarbank.example"})
        reply = message("msg-2", thread_id="thread-2", signals={"sender_domain": "northstarbank.example"})
        sent = replace(
            message("msg-3", thread_id="thread-3", sender="TestUser <me@example.com>", signals={"sender_domain": "gmail.com"}),
            label_ids=["SENT"],
        )
        classification = ai_contract("msg-1", "msg-2")
        ...
        candidate = group(
            "group-ai",
            group_key="ai-lifecycle:northstar-sent-context",
            membership_source="ai_lifecycle",
            classification=classification,
        )
    ...
    def test_conflict_resolver_gives_message_one_visible_home(self) -> None:
        messages = [message("msg-1", thread_id="thread-1"), message("msg-2", thread_id="thread-2")]
        strong = group("group-strong", group_key="ticket_id:northstarbank.example:900000002")
        weaker = replace(strong, id="group-weaker", group_key="ticket_id:northstarbank.example:900000002:duplicate", classification_confidence=0.81)
    ...
    def test_cross_entity_group_is_rejected_even_with_financial_language(self) -> None:
        messages = [
            message("msg-1", thread_id="thread-1", sender="Northstar Bank <support@northstarbank.example>"),
            message(
                "msg-2",
                thread_id="thread-2",
                sender="Interactive Brokers <brokerage-service@example.com>",
                signals={"sender_domain": "brokerage.example", "ticket_id": "900000002"},
            ),
        ]
    ...
    def test_entity_normalization_avoids_generic_sender_names(self) -> None:
        entity, channel = canonical_entity_for_message(
            message(
                "msg-1",
                thread_id="thread-1",
                sender="Default User <support@northstarbank.example>",
                signals={"sender_domain": "northstarbank.example"},
            )
        )

        self.assertEqual(entity, "Northstar Bank")
