...
class AIInboxCounterpartIdentityTests(unittest.TestCase):
    def test_verified_organization_aliases_replace_departments_and_domains(self) -> None:
        fixtures = [
            ('"Porkbun Support Dept." <support@porkbun.com>', "Porkbun"),
            ('"Screener.in" <no-reply@screener.in>', "Screener"),
            ("CreditCardOps.Maintenance@hdfcbank.bank.in", "HDFC Bank"),
            ("CEOs Office MDR <ceosoffice@hsbc.co.in>", "HSBC"),
            ("<cbsalerts.sbi@alerts.sbi.bank.in>", "SBI"),
            ('"Mando, Terri" <tbm10@psu.edu>', "Penn State"),
            ("Travel Fund Request - LF Events <travelfund@linuxfoundation.org>", "Linux Foundation"),
            ('"Charles Schwab & Co., Inc." <donotreply@email.schwab.com>', "Charles Schwab"),
            ('"bosky @aigrantsindia" <bosky@news.aigrants.in>', "AI Grants India"),
        ]
    ...
    def test_sent_message_uses_external_recipients_and_excludes_the_owner(self) -> None:
        message = _message(
            ...
            recipients={
                "to": ["Priority Redressal Credit Cards <priority@hdfc.bank.in>"],
                "cc": ["Mailbox Owner <owner@example.com>"],
            },
            label_ids=["SENT"],
        )

        self.assertEqual(ai_inbox.resolve_counterpart_entities(message), ["HDFC Bank"])
    ...
    def test_relay_resolves_represented_organization_instead_of_delivery_service(self) -> None:
        fixtures = [
            ...
            ("HDFC via Adobe Sign <agreement@adobesign.com>", "HDFC Bank"),
            ("Penn State via Calendly <invite@calendly.com>", "Penn State"),
            ("Charles Schwab via Stripe <receipt@stripe.com>", "Charles Schwab"),
        ]
        for sender, expected in fixtures:
            with self.subTest(sender=sender):
                message = _message(
    ...
    def test_personal_conversation_preserves_the_person_name(self) -> None:
        message = _message(
            ...
            sender="Alex Zhu <alex@gmail.com>",
        )

        self.assertEqual(ai_inbox.resolve_counterpart_entities(message), ["Alex Zhu"])
    ...
    def test_matter_aggregation_deduplicates_departments_and_ranks_distinct_entities(self) -> None:
        records = [
            {
                "sender": "HDFC Bank Care <care@hdfc.bank.in>",
                "recipients": {"to": ["owner@example.com"]},
                "label_ids": ["INBOX"],
                "counterpart_entities": ["HDFC"],
                "latest_message_at": "2026-08-20T10:00:00+00:00",
            },
            {
                "sender": "Priority Redressal Credit Cards <priority@hdfc.bank.in>",
                "recipients": {"to": ["owner@example.com"]},
                "label_ids": ["INBOX"],
                "counterpart_entities": ["HDFC"],
            ...
            },
            {
                "sender": "Mailbox Owner <owner@example.com>",
                "recipients": {
                    "to": ["Credit Card Operations <cards@hdfcbank.bank.in>"],
                },
                "label_ids": ["SENT"],
                "counterpart_entities": ["HDFC"],
        ...
        self.assertEqual(aggregate_counterpart_entities(records), ["HDFC Bank", "CCIL"])

    def test_group_wide_participants_collapse_departments_and_use_owner_when_needed(self) -> None:
        owner = mailbox_owner_identity(
            [
                "Gaurav Pandey <gaurav@pandey.family>",
                "Gaurav Pandey <yednapg@gmail.com>",
            ],
            fallback_display_name="Gaurav",
            fallback_address="yednapg@gmail.com",
        )
        hdfc_records = [
            {
                "sender": "Priority Redressal Credit Cards <priority@hdfc.bank.in>",
                "recipients": {"to": ["Gaurav Pandey <gaurav@pandey.family>"]},
                "label_ids": ["INBOX"],
            ...
            },
            {
                ...
                "recipients": {"to": ["Grievance-Redressalcc <cards@hdfcbank.bank.in>"]},
                "label_ids": ["SENT"],
                "counterpart_entities": ["Grievance-Redressalcc"],
                "latest_message_at": "2026-08-21T10:00:00+00:00",
            },
        ]

        self.assertEqual(
            aggregate_counterpart_entities(hdfc_records, owner_identity=owner),
            ["HDFC Bank", "Gaurav Pandey"],
        )

        sbi_record = {
            "sender": "<cbsalerts.sbi@alerts.sbi.bank.in>",
            "recipients": {"to": ["<dadajee@pandeygaurav.com>"]},
            "label_ids": ["INBOX"],
            "counterpart_entities": ["SBI", "SARTHAK SEWA PRIVATE"],
            "latest_message_at": "2026-08-20T10:00:00+00:00",
        }
        self.assertEqual(
            aggregate_counterpart_entities(
                [sbi_record],
                context_text="₹30,000 transferred to SARTHAK SEWA PRIVATE",
    ...
    def test_received_mass_mail_ignores_other_recipients(self) -> None:
        ...
        records = [
            {
                "sender": "NCSTC Network <ncstc.org@gmail.com>",
                "recipients": {
                    "to": [
                        "Gaurav Pandey <yednapg@gmail.com>",
                        "Dr Roopa Kopuri <drroopakopuri@gmail.com>",
                        "Raj Kumar <rk7571002@gmail.com>",
                    ],
                },
                "label_ids": ["INBOX"],
                "counterpart_entities": ["NCSTC Network", "Dr Roopa Kopuri"],
                "latest_message_at": "2023-09-27T11:01:00+00:00",
            },
            {
                "sender": "Tanisha <tanishathakran05@gmail.com>",
                "recipients": {
                    "to": ["NCSTC Network <ncstc.org@gmail.com>"],
                    "cc": [
                        "Gaurav Pandey <yednapg@gmail.com>",
                        "Manas <m78530556@gmail.com>",
                    ],
                },
                "label_ids": ["INBOX"],
                "counterpart_entities": ["Tanisha", "Manas"],
    ...
    def test_body_only_entities_do_not_appear_as_row_participants(self) -> None:
        ...
        record = {
            ...
            "recipients": {"to": ["Gaurav Pandey <yednapg@gmail.com>"]},
            "label_ids": ["INBOX"],
    ...
    def test_received_mail_does_not_render_the_owner_as_a_counterpart(self) -> None:
        ...
        received_record = {
            "sender": "Porkbun Support <support@porkbun.com>",
            "recipients": {"to": ["Gaurav Pandey <gaurav@pandey.family>"]},
            "label_ids": ["INBOX"],
            "counterpart_entities": ["Porkbun"],
    ...
    def test_sent_only_mail_does_not_render_the_owner_as_a_counterpart(self) -> None:
        ...
        sent_record = {
            "sender": "Gaurav Pandey <yednapg@gmail.com>",
    ...
    def test_identity_presentation_uses_proper_names_and_preserves_acronyms(self) -> None:
        message = _message(
            text="₹30,000 transferred to sarthak sewa private.",
            sender="<cbsalerts.sbi@alerts.sbi.bank.in>",
        )
        ...
        self.assertEqual(
            ai_inbox.resolve_counterpart_entities(
                message,
                suggested_entities=["sbi", "sarthak sewa private"],
                context_text=message.text_body or "",
            ),
    ...
    def test_group_wide_aliases_collapse_psu_income_tax_and_cams(self) -> None:
        ...
        fixtures = [
                ...
                [
                    {
                        ...
                        "recipients": {"to": ["gaurav@pandey.family"]},
                        "label_ids": ["INBOX"],
                        "counterpart_entities": ["PSU ENROLLMENT FEE", "Penn State"],
                    }
                ],
                ["Penn State"],
            ),
            (
                [
                    {
                        "sender": "communication@cpc.incometax.gov.in",
                        "recipients": {"to": ["gaurav@pandey.family"]},
                        "label_ids": ["INBOX"],
                        "counterpart_entities": ["Income Tax Department"],
                    },
                    {
                        "sender": "Your ITR Intimation <intimations@cpc.incometax.gov.in>",
                        "recipients": {"to": ["gaurav@pandey.family"]},
                        "label_ids": ["INBOX"],
                        "counterpart_entities": ["Your ITR Intimation"],
                    },
                ],
                ...
                [
                    {
                        ...
                        "recipients": {"to": ["yednapg@gmail.com"]},
                        "label_ids": ["INBOX"],
    ...
    def test_headline_omits_redundant_single_counterpart(self) -> None:
        self.assertEqual(
            counterpart_aware_headline(
                "HDFC confirms ₹75,000 credit-limit enhancement was processed",
                ["HDFC Bank"],
            ),
    ...
    def test_classifier_receives_only_redacted_counterpart_candidates(self) -> None:
        ...
        message = _message(
            ...
            sender="Admissions Team <private-local-part@psu.edu>",
        )
        ...
        self.assertEqual(
            ...
            [{"display_name": "Admissions Team", "domain": "psu.edu", "role": "sender"}],
        )
    ...
    def test_public_rows_add_entities_without_replacing_raw_identity_fields(self) -> None:
        matter = AIMatterRow(
            ...
            latest_message_at="2026-08-21T10:00:00+00:00",
            message_count=2,
            participants=["CreditCardOps.Maintenance@hdfcbank.bank.in"],
            counterpart_entities=["HDFC"],
            revision=1,
        )
        organizing = AIOrganizingRow(
            ...
            sender='"Porkbun Support Dept." <support@porkbun.com>',
            counterpart_entities=["Porkbun"],
            latest_message_at="2026-08-21T10:00:00+00:00",
        )

        self.assertEqual(matter.counterpart_entities, ["HDFC"])
        self.assertEqual(matter.participants, ["CreditCardOps.Maintenance@hdfcbank.bank.in"])
        self.assertEqual(organizing.counterpart_entities, ["Porkbun"])
        self.assertIn("Support Dept", organizing.sender or "")
...
class AIInboxDecisionGateTests(unittest.TestCase):
    ...
    def test_hdfc_compound_request_remains_open_after_partial_success(self) -> None:
        classification = _classification(
            ...
            subgoals=[
                ai_inbox.MatterSubgoalState(
                    ...
                    goal="Increase the HDFC credit limit",
                    status="completed",
                    latest_development="HDFC increased the credit limit by ₹75,000.",
                    evidence_message_ids=["message-1"],
                ),
                ai_inbox.MatterSubgoalState(
                    ...
                    status="waiting_on_others",
                    latest_development="HDFC has not resolved the requested upgrade.",
                    evidence_message_ids=["message-1"],
                ),
            ],
        )
