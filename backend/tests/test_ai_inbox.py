...
class AIInboxSanitizationTests(unittest.TestCase):
    ...
    def test_model_text_removes_credentials_numbers_auth_links_and_quoted_history(self) -> None:
        text = (
            "Case reference SR-1000000006 is now being processed.\n"
        ...
        _, references_without_secrets = ai_inbox._model_text(
            self.settings,
            _message(text="Case reference SR-1000000006 is now being processed."),
        )
        ...
        self.assertTrue(all("1000000006" not in value for value in references))
    ...
    def test_quoted_numeric_reply_references_in_subject_are_recognized(self) -> None:
        first = _message(
            ...
            subject="Re: '1000000005' Re: '1000000004' Requesting to upgrade credit card",
        )
        second = _message(
            text="The limit enhancement has been processed.",
            subject="RE: ‘1000000005’ Re: ‘1000000004’ Limit enhancement processed",
        )
        ...
        self.assertTrue(all("1000000005" not in item for item in first_references))
    ...
    def test_unlabelled_mixed_identifier_is_subject_only(self) -> None:
        _, subject_references = ai_inbox._model_text(
            self.settings,
            _message(text="A support update.", subject="Re: HSBC///FB-1000000006"),
        )
