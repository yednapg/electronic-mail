...
| Native compile/tests | `npm run macos:typecheck && npm run macos:test` | **Passed (local): test build succeeded; 200/200 tests, 0 failures, 2026-07-23.** xcresult: `/tmp/ElectronicMailDerivedData/Logs/Test/Test-ElectronicMail-2026.07.23_20-24-48-+0530.xcresult` |
...
| Backend unit/integration tests | `npm run backend:test`; `APP_ENV=staging DATABASE_URL=postgresql://localhost/electronic_mail npm run backend:test` | **Passed (local), 2026-07-23:** the default runner passed 319 tests with 17 staging-only skips; the staging/Postgres run passed 319 tests with 0 skips |
