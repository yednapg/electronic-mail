...
| 2 | Reproducible dependencies and toolchains | `npm ci`; `npm run reproducibility:verify`; reviewed lock diff | **Passed (local):** manifests match `package-lock.json`, direct Node dependencies are exact, Python is fully version-pinned, Node/Python runtimes are exact, Docker bases/frontends and Actions are digest/commit-pinned |
| 3 | CI, security, and immutable build policy | Successful `Quality and release gates` push run for the exact release SHA; npm and Python audits; container jobs | Configured; local policy/web/database/launch-verifier checks passed, but no exact-commit hosted CI run exists for uncommitted changes and a fresh registry-backed npm audit remains pending |
...
| 5 | Backup, restore, and disaster recovery | Managed PITR configuration; `npm run database:scripts:test`; encrypted logical backup; isolated restore drill record | **Passed (local)** for deterministic scripts. Actual managed PITR, off-host encrypted copies, and quarterly isolated restore drill are **Pending external** |
| 6 | Universal signed/notarized macOS artifact | `npm run release:macos:preflight`; protected release workflow; `scripts/verify-macos-release.sh`; notarization/stapling/Gatekeeper evidence | Pending candidate and Apple publisher credentials |
...
|---|---|---|
| Reproducibility policy | `npm run reproducibility:verify:test && npm run reproducibility:verify` | **Passed (local): 8/8 fail-closed policy tests plus repository verification, 2026-07-23** |
...
| Native compile/tests | `npm run macos:typecheck && npm run macos:test` | **Passed (local): test build succeeded; 173/173 tests, 0 failures, 2026-07-23.** xcresult: `/tmp/ElectronicMailDerivedData/Logs/Test/Test-ElectronicMail-2026.07.23_15-32-43-+0530.xcresult` |
| Universal unsigned policy preflight | `VERSION=1.0.0 BUILD_NUMBER=1 BACKEND_URL=https://api.electronicmail.app npm run release:macos:preflight` | **Passed (local), 2026-07-23:** universal `x86_64` + `arm64`, identity-free Release policy. This is not signing/notarization evidence; distributable release still requires stable public Xcode (only Xcode-beta is installed locally) |
| Backend unit/integration tests | `npm run backend:test`; `APP_ENV=staging DATABASE_URL=postgresql://localhost/electronic_mail npm run backend:test` | **Passed (local), 2026-07-23:** the default runner passed 294/294 tests with 12 staging-only skips; the staging/Postgres run passed 294/294 with 0 skips |
| Local runtime readiness/auth-failure behavior | `bash scripts/dev-backend-runtime.sh`; HTTP checks for `/health` and `/ready`; worker/poller observation with a revoked Google refresh credential | **Passed (local), 2026-07-23:** `/health` and `/ready` returned HTTP 200, credential expiry was classified as reauthorization-required, queued work was cancelled safely, and the prior recurring auth retry storm did not recur |
...
| Python production audit | `python -m pip_audit -r backend/requirements.lock` | Not evidenced: networked vulnerability lookup was unavailable in this environment |
...
- Stable public Xcode, Developer ID signing, notarization/stapling, Gatekeeper validation, and the clean-Mac install/upgrade matrix for the exact DMG.
