...
| 3 | CI, security, and immutable build policy | Successful `Quality and release gates` push run for the exact release SHA; npm and Python audits; container jobs | Configured; local policy/web/database/launch-verifier checks passed. Baseline hosted run [30035806677](https://github.com/yednapg/electronic-mail/actions/runs/30035806677) for `13a63714f6310fb9e4fa0c720879917350131590` correctly failed and exposed runner/dependency/container blockers now remediated in this worktree. The exact reviewed commit still needs a green hosted rerun and fresh registry-backed npm and Python audits |
...
| Installed production dependency graph | Canonical Node 22.22.0/npm 10.9.4 `npm ci`; lock/override verification; production build/runtime | **Partially passed (local), 2026-07-24:** the exact lock and reviewed PostCSS/Sharp security overrides pass policy and the production build/runtime. This host's existing Node 26 install reports the intentionally overridden Next dependency declarations plus platform-specific optional/extraneous packages, so a clean canonical `npm ci` graph remains hosted-CI evidence and is not claimed locally |
| Native compile/tests | `npm run macos:typecheck && npm run macos:test` | **Passed (local): test build succeeded; 216/216 tests, 0 failures, 2026-07-24.** xcresult: `/tmp/ElectronicMailDerivedData/Logs/Test/Test-ElectronicMail-2026.07.24_01-32-01-+0530.xcresult` |
...
| Backend unit/integration tests | `npm run backend:test`; `APP_ENV=staging DATABASE_URL=postgresql://localhost/electronic_mail npm run backend:test` | **Passed (local), 2026-07-24:** the default runner passed 435 tests with 20 staging-only skips; the staging/Postgres run passed 435 tests with 0 skips |
...
| Beta distribution policy | `npm run release:macos:beta:test` | **Passed (local): 18/18 policy, coinstallation-safety, and exact-DMG-layout tests, 2026-07-24.** A freshly committed build remains pending |
...
| Exact-commit hosted CI | Green `.github/workflows/quality.yml` push run for release SHA | Baseline run 30035806677 failed as designed and its observed blockers are fixed locally; pending reviewed exact-SHA push and green hosted CI. Repository Actions now require full-length SHA pins |
