...
| 2 | Reproducible dependencies and toolchains | `npm ci`; `npm run reproducibility:verify`; reviewed lock diff | **Passed for private Beta 6 source SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`, 2026-07-24:** an isolated clean tree passed `npm ci`, `npm ls --all`, hash-only Python installation, and the full verifier under exact Node 22.22.0, npm 10.9.4, and Python 3.12.13. Hosted run [30079976742](https://github.com/yednapg/electronic-mail/actions/runs/30079976742) separately passed the reproducibility job and the available Python 3.12.10 macOS target. The final production release SHA must repeat this gate |
| 3 | CI, security, and immutable build policy | Successful `Quality and release gates` push run for the exact release SHA; npm and Python audits; container jobs | **Passed for private Beta 6 source SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`, 2026-07-24:** hosted run [30079976742](https://github.com/yednapg/electronic-mail/actions/runs/30079976742) passed all six backend, web, macOS, reproducibility, and container jobs, including fresh dependency audits. The final signed production release SHA must repeat this gate |
...
| 6 | Universal signed/notarized macOS artifact | Approved Xcode 16.4 (16F6); `npm run release:macos:preflight`; protected release workflow; `scripts/verify-macos-release.sh`; notarization/stapling/Gatekeeper evidence | A universal, ad-hoc-signed, unnotarized private [Beta 6](https://github.com/yednapg/electronic-mail/releases/tag/v0.1.0-beta.6) passed static and mounted-DMG launch verification. Production remains pending the approved stable toolchain and Apple publisher credentials |
...
| Native compile/tests | `npm run macos:typecheck && npm run macos:test` | **Passed (local): test build succeeded; 218/218 tests, 0 failures, 2026-07-24.** xcresult: `/tmp/ElectronicMailDerivedData/Logs/Test/Test-ElectronicMail-2026.07.24_08-27-06-+0530.xcresult` |
...
| Local runtime readiness/auth-failure behavior | `bash scripts/dev-backend-runtime.sh`; HTTP checks for `/health` and `/ready`; worker/poller observation with a revoked Google refresh credential; Computer Use smoke of the rebuilt native app | **Passed (local), 2026-07-24:** `/health` and `/ready` returned HTTP 200 with AI disabled and the current schema head; credential expiry remained reauthorization-required without a retry storm. The rebuilt signed-in app relaunched without losing its session, defaulted to a hidden sidebar, rendered a known read message from a 5.12 ms reader response, showed/hid its sidebar with one bounded count sweep, and completed/cleared native search from a 103.75 ms response. Release-mode Instruments captures of real inbox and rich-reader scrolling recorded no hangs and no actionable SwiftUI body/update bottleneck; the only isolated annotations lacked an attributable view or code path |
...
| Beta distribution policy | `npm run release:macos:beta:test` | **Passed, 2026-07-24:** 31/31 policy, coinstallation-safety, exact-DMG-layout, exact-SHA publishing, byte-verification, and fixed-`github.com` routing tests passed. Private [Beta 6](https://github.com/yednapg/electronic-mail/releases/tag/v0.1.0-beta.6) was built from exact green SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`; its universal mounted DMG passed both launch smokes and was published only after pre- and post-publication verification. DMG SHA-256: `49b81d0c7a90d27b92fc36916d7a8427bd939a41b6a591e451d892a76990f2b6` |
...
| Backend container build/readiness | Exact `backend/Dockerfile` image, migrated Postgres, `/ready` assertions | **Passed in hosted run [30079976742](https://github.com/yednapg/electronic-mail/actions/runs/30079976742) for private Beta 6 source SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`** |
| Web container/runtime boundary | Exact `Dockerfile.web` image, legal pages, headers, native-only 404 boundary | **Passed in hosted run [30079976742](https://github.com/yednapg/electronic-mail/actions/runs/30079976742) for private Beta 6 source SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`** |
| Exact-commit hosted CI | Green `.github/workflows/quality.yml` run for release SHA | **Passed for private Beta 6 source SHA `456fe53cc6c5a961905daa9e3798d53f61b6188d`: all six jobs passed in hosted run [30079976742](https://github.com/yednapg/electronic-mail/actions/runs/30079976742).** The final production release SHA must repeat this exact-SHA gate. Repository Actions require full-length SHA pins |
...
- A successful hosted CI run for the final signed production release SHA, including fresh npm and Python vulnerability audits and container gates. Private Beta 6 already passed this gate for its exact source SHA.
