...
| 2 | Reproducible dependencies and toolchains | `npm ci`; `npm run reproducibility:verify`; reviewed lock diff | **Passed (local), 2026-07-24:** an isolated clean tree passed `npm ci`, `npm ls --all`, hash-only Python installation, and the full verifier under exact Node 22.22.0, npm 10.9.4, and Python 3.12.13. Hosted macOS automation separately pins available Python 3.12.10 on `macos-15-intel`; all 53 runtime and 29 audit-tool pins also passed hash-only binary resolution for that target. Exact-SHA hosted CI remains pending |
| 3 | CI, security, and immutable build policy | Successful `Quality and release gates` push run for the exact release SHA; npm and Python audits; container jobs | Configured; local policy/web/database/launch-verifier checks pass, npm reports zero vulnerabilities, and isolated audits of both Python locks report no known vulnerabilities. Baseline hosted run [30035806677](https://github.com/yednapg/electronic-mail/actions/runs/30035806677) for `13a63714f6310fb9e4fa0c720879917350131590` exposed blockers now remediated locally. The exact reviewed commit still needs a green hosted rerun and container evidence |
...
| Beta distribution policy | `npm run release:macos:beta:test` | **Passed (local): 31/31 policy, coinstallation-safety, exact-DMG-layout, exact-SHA publishing, byte-verification, and fixed-`github.com` routing tests, 2026-07-24.** A freshly committed build remains pending |
