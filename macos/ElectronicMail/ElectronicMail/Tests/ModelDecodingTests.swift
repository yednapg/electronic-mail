...
final class ModelDecodingTests: XCTestCase {
    ...
    func testMailboxRowDisplaySenderPreservesEmailLocalPartCapitalization() {
        let lowercase = makeMailboxPresentationRow(sender: "cbsalerts.sbi@alerts.sbi.bank.in")
        let mixedCase = makeMailboxPresentationRow(sender: "HDFCFXclearretail@mailer.example.com")

        XCTAssertEqual(lowercase.displaySender, "cbsalerts sbi - alerts.sbi.bank.in")
        XCTAssertEqual(mixedCase.displaySender, "HDFCFXclearretail - mailer.example.com")
    }
          ...
          <body style="margin:0;background:#f5f5f5">
            <table class="renewal-card" width="100%" role="presentation" style="max-width:640px;margin:0 auto;background:#ffffff">
              <tbody>
                ...
                <tr><td class="button-row" style="padding:24px 32px 32px"><a href="https://porkbun.example/account" style="display:inline-block;padding:12px 18px;background:#ef476f;color:#ffffff">Review domains</a></td></tr>
              </tbody>
            </table>
          </body>
        </html>
    ...
    func testAIInboxResponseDecodesSeparateMatterAndOrganizingRows() throws {
        let data = Data(
            """
            {
              ...
              "matters": [{
                "id": "matter-1",
                "kind": "matter",
                "title": "HSBC processed the branch-linking request",
        ...
        XCTAssertEqual(response.organizing.first?.counterpartEntities, ["Porkbun"])

        let legacyJSON = String(decoding: data, as: UTF8.self)
            .replacingOccurrences(of: "\"counterpart_entities\": [\"HSBC\", \"CCIL\"],", with: "")
            .replacingOccurrences(of: "\"counterpart_entities\": [\"Porkbun\"],", with: "")
    ...
    func testAIInboxCounterpartFormattingPrefersEntitiesAndDeduplicatesLabels() {
        XCTAssertEqual(
            AIInboxParticipantFormatting.counterpartLabel(["HDFC", "hdfc"]),
            "HDFC"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            AIInboxParticipantFormatting.counterpartLabel(["HSBC", "CCIL", "HDFC"]),
            "HSBC, CCIL, and HDFC"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            AIInboxParticipantFormatting.listLabel(
                counterpartEntities: nil,
                fallbackParticipants: ["HDFC Bank", "Gaurav Pandey"],
                emptyLabel: "Waiting"
            ),
            "HDFC Bank and Gaurav Pandey"
        )
    }
