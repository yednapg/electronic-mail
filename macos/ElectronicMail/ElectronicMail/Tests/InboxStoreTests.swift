...
final class InboxStoreTests: XCTestCase {
    ...
    func testSmartInboxGroupAcrossSeparateMailboxRowsExpandsWithSyntheticChildren() async {
        let firstRawRow = makeMailboxRow(
            threadID: "northstar-wire-thread",
            latestSourceRecordID: "northstar-wire-message",
            receivedAt: "2026-06-07T10:00:00+05:30",
            title: "Northstar international wire registered"
        )
        let secondRawRow = makeMailboxRow(
            threadID: "northstar-response-thread",
            latestSourceRecordID: "northstar-response-message",
            receivedAt: "2026-06-07T11:00:00+05:30",
            title: "Northstar response on wire request",
            hasAttachments: true,
            attachmentCount: 1
        )
        let mailbox = MailboxResponse(
        ...
        let smartRow = SmartInboxRow(
            id: "smart-row-northstar-wire",
            rowKey: "mail-object:northstar-wire",
            rowType: "verified_group",
            title: "Wire transfer status with Northstar Bank",
            summary: "Two Northstar messages track the same wire transfer request.",
            primarySender: "Northstar Bank Care <support@northstarbank.example>",
            latestMessageAt: "2026-06-07T11:00:00+05:30",
            latestMessageID: "northstar-response-message",
            readerThreadID: "smart-row:smart-row-northstar-wire",
            sourceThreadIDs: ["northstar-wire-thread", "northstar-response-thread"],
            sourceMessageIDs: ["northstar-wire-message", "northstar-response-message"],
        ...
        XCTAssertEqual(store.flatRows.map(\.id), ["smart-row-northstar-wire"])
        XCTAssertEqual(store.flatRows.first?.threadID, "smart-row:smart-row-northstar-wire")
        XCTAssertEqual(store.flatRows.first?.title, "Wire transfer status with Northstar Bank")
        XCTAssertEqual(store.flatRows.first?.isGrouped, true)
        XCTAssertEqual(store.flatRows.first?.isExpandable, true)
        XCTAssertEqual(store.flatRows.first?.messageCount, 2)
        XCTAssertEqual(store.flatRows.first?.hasAttachments, true)

        store.toggleExpansion(threadID: "smart-row:smart-row-northstar-wire")

        XCTAssertEqual(store.flatRows.map(\.id), [
            "smart-row-northstar-wire",
            "smart-row:smart-row-northstar-wire::message::northstar-wire-message",
            "smart-row:smart-row-northstar-wire::message::northstar-response-message"
        ])
        XCTAssertEqual(store.flatRows[1].title, "Northstar international wire registered")
        XCTAssertEqual(store.flatRows[2].title, "Northstar response on wire request")
        XCTAssertEqual(store.flatRows[2].focusedMessageID, "northstar-response-message")
    }
    ...
    func testMailboxRefreshDropsStaleCachedGroupsInsideRefreshedFirstPageWindow() {
        ...
        let refreshedRow = makeMailboxRow(
            ...
            title: "State University admissions visa document next steps"
        )
        let current = MailboxResponse(
            label: .inbox,
            totalThreads: 2,
...
private func makeSmartInboxRow() -> SmartInboxRow {
    SmartInboxRow(
        ...
        primarySender: "Apple Developer <apple-developer@example.com>",
